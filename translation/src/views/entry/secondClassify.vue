<template>
  <CustomModal :modalWidth="modalWidth" :visible="visible" :modalTitle="modalTitle" :showOk="false" cancelText="关闭" @handleClose="handleClose"
    ...
    <div style="width:100%;height:350px">
      <a-form :model="search" layout="inline" autocomplete="off">
        <a-form-item label="一级分类" name="operator">
          <a-select v-model:value="search.classify1" style="width: 200px" placeholder="请选择" :fieldNames="{label:'title',value:'key'}"
            :options='classfy1Option' @select="classify1Select" allowClear>
          </a-select>
        </a-form-item>
        <a-form-item>
      ...
      <div class="table">
        <a-table class="ant-table-striped" :columns="columns" :data-source="dataSource" :scroll="{x:'100%' , y: '280px'}" :pagination='false'
          :row-class-name="(_record, index) => (index % 2 === 1 ? 'table-striped' : null)"
          :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" :row-key="record => record.id" ref="secondClassifyTable"
          bordered>
          <template #bodyCell="{ column, record, text }">
            <template v-if="column.dataIndex === 'name'">
              <div class="editable-cell">
                <template v-if="editableData[record.id]">
                  <a-input v-model:value="editableData[record.id][column.dataIndex]" style="margin: -5px 0;width:80%" />
...
export default {
  ...
  methods: {
    ...
    add() {
      if (!this.search.classify1 || this.search.classify1 === "") {
      ...
      this.$nextTick(() => {
        let container =
          this.$refs.secondClassifyTable.$el.querySelector(".ant-table-body");
