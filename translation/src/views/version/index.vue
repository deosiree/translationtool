    ...
    bordered>
        <template #bodyCell="{ column, text, record }">
            ...
            <template v-else-if="column.dataIndex === 'isDefault'">
                ...
                :disabled="!authority.includes('updateVersionInfo')"
                @change="handelDefaultChange(record)"
                />
            </template>
            <template v-else-if="column.dataIndex === 'isSelect'">
                <a-switch 
                :checked="editableData[record.id] ? editableData[record.id].isSelect : record.isSelect" 
                :checked-value=1
                :unchecked-value=0
                size="small" 
                checked-children="是" 
            ...
            <template v-else-if="column.dataIndex === 'operation'">
                ...
                <span v-else>
                    <a-button type="primary" ghost size="small" v-if="authority.includes('updateVersionInfo')"
...
export default {
    ...
    data() {
        return{
            ...
            columns:[
                ...
                {title: '是否默认版本', dataIndex: 'isDefault',align:'center',width: '15%'},
                {title: '是否可选版本',dataIndex: 'isSelect',align:'center',width: '15%'},
    ...
    methods: {
        ...
        getAuthority(){
            ...
            // 权限中含有 编辑  删除 表格展示操作栏
            if(this.authority.includes('updateVersionInfo') || this.authority.includes('deleteVersionInfo')){
        ...
        //保存
        save(id){
            if(this.editableData[id].name === '' || this.editableData[id].name === null){
        ...
        handleAdd(){
            ...
            // 滚动到最底部
            this.$nextTick(()=>{
                let container = this.$refs.versionTable.$el.querySelector('.ant-table-body')
        ...
        handelDefaultChange(record){
            ...
            if(!this.editableData[record.id]){
                ...
                updateVersionInfo(record).then((res) => {
