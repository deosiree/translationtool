<template>
  <Modal :modalWidth="modalWidth" :visible="visible" :modalTitle="modalTitle" @handleClose="handleClose" @handleOK="handleOK"
    ...
    <div class="content">
      <a-form ref="dictTermRef" name="custom-validation" autocomplete='off' :model="dictTerm" :label-col="labelCol">
        <a-form-item label="词条" name="entry" :rules="[{ required: true, message: '请输入词条!' }]">
        ...
        <a-form-item label="翻译语种" name="lang" :rules="[{ required: true, message: '请选择!' }]">
          <a-select v-model:value="dictTerm.lang" placeholder="请选择" :options='translateTypes' :fieldNames="{label:'name',value:'code'}"
            @change="tranlateChange" allowClear>
          </a-select>
        </a-form-item>
        <a-form-item label="翻译" name="translation" :rules="[{ required: true, message: '请输入内容!' }]">
...
import { addDicTerm, updateDicTrans } from "@/http/api/i18Server";
import { getLanguage } from "@/http/api/entry";
export default {
  components: {
    Modal,
  },
