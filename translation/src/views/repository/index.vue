<template>
<div style="width:100%;height:100%;padding:10px;" ref="box">
            ...
            <a-row>
                <a-col :span="16">
                    ...
                    >
                        ...
                        name="version"
                        >
                            <a-input v-model:value="search.version" placeholder="请输入版本名称" style="width:250px"></a-input>
                        </a-form-item>
                    </a-form>
                </a-col>
                <a-col :span="8">
                    <div class="operation">
                        <a-button type="primary" size="middle" @click="getVersionList">查询</a-button>
                        <a-button type="primary" size="middle" @click="reset" style="background-color:#36BF7D;border:#36BF7D">重置</a-button>
                        <AddRepository :title="repositoryBtnTitle" :icon="repositoryBtnIcon" @afterClose="getVersionList"/>
                        <a-button type="primary" size="middle" @click="deleteBatch">
    ...
    >
        <template #bodyCell="{ column, text, record }">
            <template v-if="['version','remark'].includes(column.dataIndex)">
                <div>
            ...
            <template v-else-if="column.dataIndex === 'operation'">
                ...
                <span v-else>
                    <a-button type="primary" ghost size="small" @click="detail(record)">查看</a-button>
                    <!-- <a-button type="primary" ghost size="small"
                    @click="edit(record.id)">编辑</a-button> -->
                    <a-popconfirm title="确认删除?" ok-text='是' cancel-text='否' @confirm="deleteRepository(record.id)">
    ...
    data() {
        return{
            ...
            columns:[
                ...
                {title: '版本名称',dataIndex: 'version',align:'center'},
            ...
            repositoryBtnTitle:'新增',
            repositoryBtnIcon:'PlusOutlined',
            loading:false,
            operationFlag: false,
            detailVisible: false
        }
    ...
    methods: {
        ...
        deleteBatch(){
            ...
            Modal.confirm({
                ...
                cancelText: '取消',
                onOk: () => {
                    batchDeleteVersionTable(this.selectedRowKeys).then((res) => {
        ...
        deleteRepository(id){
            let data = [id]
            batchDeleteVersionTable(data).then((res) => {
                message.success("删除成功！")
                 this.getVersionList()
            })
        },
