<template>
  <div style="width:100%;height:100%;" ref="box">
        ...
        <a-row>
          <a-col :span="16">
            <a-form :model="search" name="horizontal_login" layout="inline" autocomplete="off">
              <a-form-item label="标签" name="labelName">
                <a-input v-model:value="search.labelName" placeholder="请输入标签"></a-input>
              </a-form-item>
            </a-form>
          </a-col>
          <a-col :span="8">
            <div class="operation">
              <a-button type="primary" size="middle" @click="getLabelList">查询</a-button>
              <a-button type="primary" size="middle" @click="reset" style="background-color:#36BF7D;border:#36BF7D">重置</a-button>
              <a-button type="primary" size="middle" @click="handleAdd" v-if="authority.includes('addLabel')">
                <template #icon>
                  <PlusOutlined />
                </template>
                新增
              </a-button>
              <a-button type="primary" size="middle" @click="deleteBatch" v-if="authority.includes('deleteLabel')">
                <template #icon>
                  <DeleteOutlined />
                </template>
                批量删除
    ...
    <a-table class="ant-table-striped" :columns="columns" :data-source="dataSource" :customRow="doubleClick"
      ...
      :pagination='false' :row-class-name="(_record, index) => (index % 2 === 1 ? 'table-striped' : null)" ref="labelTable" bordered>
      <template #bodyCell="{ column, text, record }">
        <template v-if="['labelName'].includes(column.dataIndex)">
          <div>
        ...
        <template v-else-if="column.dataIndex === 'operation'">
          <div class="editable-row-operations">
            ...
            <span v-else>
              <a-button type="primary" ghost size="small" v-if="authority.includes('updateLabel')" @click="edit(record.id)">编辑</a-button>
              <a-popconfirm title="确认删除?" ok-text='是' cancel-text='否' @confirm="deleteLabel(record.id)" v-if="authority.includes('deleteLabel')">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: "标签", dataIndex: "labelName", align: "center" },
  ...
  methods: {
    ...
    getAuthority() {
      ...
      // 权限中含有 编辑  删除  表格展示操作栏
      if (
        this.authority.includes("updateLabel") ||
        this.authority.includes("deleteLabel")
      ) {
    ...
    save(id) {
      ...
      if (
        this.editableData[id].labelName === "" ||
        this.editableData[id].labelName === null
      ) {
      ...
      if (id.startsWith("new")) {
        //调用新增接口
        addLabel(this.editableData[id]).then((res) => {
        ...
        //调用修改接口
        updateLabel(this.editableData[id]).then((res) => {
    ...
    doubleClick(record, index) {
      return {
        onDblclick: (event) => {
          if (this.authority.includes("updateLabel")) {
    ...
    handleAdd() {
      ...
      this.$nextTick(() => {
        let container =
          this.$refs.labelTable.$el.querySelector(".ant-table-body");
    ...
    deleteBatch() {
      ...
      Modal.confirm({
        ...
        cancelText: "取消",
        onOk: () => {
          deleteLabel(this.selectedRowKeys).then((res) => {
