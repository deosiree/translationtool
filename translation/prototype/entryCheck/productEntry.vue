...
import { checkMaxLength, checkSpecialChar } from "@prototype/entryCheck/http/api";
import {
  PlusOutlined,
  DeleteOutlined,
  CopyOutlined,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      searchTypes: [
        ...
        { label: "特殊字符校验", value: "checkSpecialChar" },
        { label: "长度超限校验", value: "checkMaxLength" },
  ...
  methods: {
    ...
    conditionalQuery(accurate = [], isInit = false) {
      ...
      } else if (this.search.searchType == "checkSpecialChar") {
        // 特殊字符校验（mock api，刷新列表）
        this.lastSearchType = "checkSpecialChar";
        this.lastAccurate = [];
        this.getCheckSpecialChar();
      } else if (this.search.searchType == "checkMaxLength") {
        // 长度超限校验（mock api，刷新列表）
        this.lastSearchType = "checkMaxLength";
        this.lastAccurate = [];
        this.getCheckMaxLength();
    ...
    async getCheckSpecialChar() {
      ...
      try {
        const params = {
          classfyID: this.currentProduct && this.currentProduct.key,
          search: this.search,
        };
    ...
    async editSave(id) {
      ...
      try {
        ...
        if (id.startsWith("new") || id.startsWith("copy")) {
          ...
          this.confirmSave([this.editableData[id]]);
        }
    ...
    refreshTable() {
      ...
      if (this.lastSearchType === "checkSpecialChar") {
        this.getCheckSpecialChar();
      } else if (this.lastSearchType === "checkMaxLength") {
        this.getCheckMaxLength();
