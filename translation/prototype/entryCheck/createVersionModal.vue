<template>
  <CustomModal :modalWidth="modalWidth" modalTitle="批量选择" :visible="visible" :showCancel="false" :showOk="false"
    :fullFlag="true" cancelText="取消" @handleClose="handleClose" @afterClose="afterClose"
    ...
    <template v-slot:leftBottomBtn>
      ...
      <WriteBackButton buttonTitle="回写（原型）" size="middle" :submitMode="'emit'" :dataSource="dataSource"
        :enableValidation="true" :isPrototype="true"
        @update:dataSource="$emit('update:dataSource', $event)" />
      <a-button type="primary" danger @click="openRedundancyCheck">冗余校验</a-button>
      <!-- 保留原有按钮，便于增量演示；原型模式下仍受原始 v-if 控制 -->
  ...
  <!-- 回写校验模态框（原型：在“已选词条”弹窗内打开） -->
  <WriteBackValidateModal mode="modal" :visible="validateModalOpen" :validationData="validationTableData"
    :deleteMode="'soft'" :writeBackParams="lastWriteBackForm" :isPrototype="true" :dataSource="dataSource"
    @close="validateModalOpen = false" @update:dataSource="$emit('update:dataSource', $event)" />

  <!-- 冗余校验（原型）：由“冗余校验”按钮触发，使用硬删除模式 -->
  <WriteBackValidateModal mode="modal" :visible="redundancyVisible" :validationData="redundancyTableData"
    :deleteMode="'hard'" :writeBackParams="lastWriteBackForm" :isPrototype="true" :dataSource="dataSource"
    @close="redundancyVisible = false" @update:dataSource="$emit('update:dataSource', $event)" />
...
import ClearFields from "@prototype/entryCheck/clearFields.vue";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      validationTableData: [],
      lastWriteBackForm: null,

      // ==================== 原型：冗余校验弹窗状态 ====================
      redundancyVisible: false,
      redundancyTableData: [],
    };
  },
  ...
  methods: {
    // 移除某条已选词条（每行最右边的按钮
    remove(record) {
      const newdataSource = Array.isArray(this.dataSource)
        ? this.dataSource.filter((item) => item && item.id !== record.id)
        : [];

      // 原型：保证“取消选择”在缺少 product/selectedProducts 等上下文时也能工作
      this.$emit("update:dataSource", newdataSource);

      // 兼容 selectedRowKeys 既可能是 [id] 也可能是 [{id}]
      if (Array.isArray(this.selectedRowKeys)) {
        const newSelectedRowKeys = this.selectedRowKeys.filter((item) => {
          if (item && typeof item === "object") return item.id !== record.id;
      ...
      if (Array.isArray(this.selectedRows)) {
        const newSelectedRows = this.selectedRows.filter((item) => item && item.id !== record.id);
        this.$emit("update:selectedRows", newSelectedRows);
      }

      // 若父组件传入 selectedProducts/product，则保持原逻辑；否则跳过
      if (
        this.selectedProducts &&
        this.selectedProducts.products &&
        typeof this.selectedProducts.products.size === "number" &&
        this.product &&
        this.product.key
      ) {
    ...
    // ==================== 原型：回写校验（复用 WriteBackValidateModal） ====================
    handleOpenValidation(payload) {
      this.lastWriteBackForm = payload && payload.writeBack ? payload.writeBack : null;
      // 原型模式：将当前已选词条(dataSource)作为 dataA 传入，生成校验结果时复用其 id，便于在校验弹窗中对齐 dataA
      this.validationTableData = buildMockValidationResult(this.dataSource || []);
    ...
    // ==================== 原型：冗余校验（硬删除模式） ====================
    openRedundancyCheck() {
      // 同样复用 dataSource 的 id，便于观察“硬删除”对 dataA 的影响
      this.redundancyTableData = buildMockValidationResult(this.dataSource || []);
      this.redundancyVisible = true;
      message.success("冗余校验数据已生成（原型）");
    },
    // 创建版本
    handleOK() {
