    ...
    data(){
        return{
            ...
            columns: [
                ...
                //     return text.index + 1
                // },fixed: 'left'},
                {title: 'Abbr',dataIndex: 'abbr',width:150,fixed: 'left',
                    customCell: (record, rowIndex, column) => ({
                        colSpan: record.flag != 'col' ? 1 : 17,
                    }),},
                {title: '版本',dataIndex: 'version',width:150,align:'center',fixed: 'left',
    ...
    methods:{
        ...
        versionSelect(record){
            ...
            searchEntry(data,params).then((res) => {
                ...
                if(list.length > 0){
                    Object.assign(this.dataSource.filter(item => record.id === item.id)[0], list[0]);
        ...
        getEntryToVersion(){
            ...
            if(this.search.classify != undefined){
                this.search.classify.forEach((item) => {
                    classifyKeys = classifyKeys.concat(this.getChild(this.treeData,item))
                })
            }
            getEntryToVersion(params,classifyKeys).then((res) => {
                ...
                if(res.data.versionEntries.length === 0){
                    this.dataSource = res.data.versionEntries.concat(res.data.fuzzyEntries)
                }else{
                    // 增量升级时 表格内容分类展示
                    this.dataSource = []
                    let versionEntries = {
