    ...
    <div class="json-edit-modal">
      ...
      <div class="json-pane">
        ...
        <div class="json-pane-body">
          ...
          <div class="json-code-wrapper json-code-list">
            <a-checkbox-group v-model:value="selectedJsonLineIds" class="json-checkbox-group">
              <div
                v-for="line in jsonImportLines"
                :key="line.id"
                class="json-code-row"
                :class="{ 'json-code-row--selected': selectedJsonLineIds.includes(line.id) }"
              >
                <a-checkbox
                  class="json-line-checkbox"
                  :value="line.id"
...
export default {
  ...
  data() {
    return {
      ...
      // 行级 JSON 视图与勾选（基于格式化 JSON 文本，每行一个条目）
      jsonImportLines: [], // [{ id, index, text, selectable, key, value }]
      selectedJsonLineIds: [],
    };
  },
  computed: {
  ...
  methods: {
    ...
    openJsonEditModal(closeLoading) {
      ...
      return this.$refs.configFormRef
        ...
        .then(() => {
          ...
          this.jsonImportSourceContent =
            `// 导入配置文件（JSON）：${fileLabel}\n` + MOCK_JSON_IMPORT_CONTENT;

          // 抽取区初始化
          this.jsonExtractContent = MOCK_JSON_EXTRACT_INITIAL;

          // 基于当前 JSON 文本构建“按行”的只读视图（不依赖全局 JSON 解析，可展示全部字符与空行）
          this.buildJsonImportLinesFromSource();
    ...
    closeJsonEditModal() {
      ...
      this.jsonImportLoading = false;
      this.selectedJsonLineIds = [];
      this.jsonImportLines = [];
    },
    // ==================== JSON 配置文件导入：辅助方法与交互 ====================
    // 将 jsonImportSourceContent 按行拆分为行数组，保留所有空格与空行
    buildJsonImportLinesFromSource() {
      const content = this.jsonImportSourceContent || "";
      const rawLines = content.split(/\r?\n/);
      const lines = rawLines.map((text, idx) => {
        const index = idx + 1;
        const parsed = this.parseJsonLineForStringValue(text);
        return {
          id: `json-line-${index}`,
          index,
          text,
      ...
      this.jsonImportLines = lines;
      this.selectedJsonLineIds = [];
    },
    // 尝试从单行 JSON 文本中解析出「可翻译的字符串值」
    // 仅当行形如 `"key": "xxx",` 或 `"key": "xxx"` 时才视为可选
    parseJsonLineForStringValue(lineText) {
      const original = lineText == null ? "" : String(lineText);
      const trimmed = original.trim();
      // 空行或注释行不可选
      if (!trimmed || trimmed.startsWith("//")) {
        return { selectable: false, key: null, value: null };
      }
      // 粗略匹配 key:value 形式
    ...
    handleJsonNeedTranslate() {
      if (!this.selectedJsonLineIds.length) {
      ...
      setTimeout(() => {
        const idSet = new Set(this.selectedJsonLineIds);
        const selectedLines = this.jsonImportLines.filter(
          (line) => idSet.has(line.id) && line.selectable
        );
        ...
        selectedLines.forEach((line) => {
          // 使用 key + 行号 作为键，避免重复；值为原始字符串值
          const extractKey = line.key
            ? `${line.key}@line_${line.index}`
            : `line_${line.index}`;
          currentExtract[extractKey] = line.value;
        });
...
/* 左侧 JSON 行级视图：一行一个展示行，行号与勾选框贴左对齐 */
.json-code-list {
  padding: 4px 0;
}
