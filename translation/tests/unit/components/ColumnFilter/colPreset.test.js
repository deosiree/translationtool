...
describe('colPreset', () => {
  ...
  it('长度列 key 与 label 对齐：模块上限用 maxByte/foreignMaxByte，旧别名 hidden，zhCharLength 保留', async () => {
    ...
    expect(productByValue.maxByte.label).toBe('中文字符上限')
    expect(productByValue.foreignMaxByte.label).toBe('外文字符上限')
    ...
    expect(wbByValue.foreignMaxByte.label).toBe('外文字符上限')
    expect(wbByValue.maxByte.label).toBe('中文字符上限')
